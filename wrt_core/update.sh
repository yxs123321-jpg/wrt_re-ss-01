...
modify_kernel_size() {
    ...
    for device in "${devices[@]}"; do
        if grep -q "Device/${device}" "$ipq60xx_mk_path"; then
            sed -i "/Device\/${device}/,/endef/{s/KERNEL_SIZE := 6144k/KERNEL_SIZE := 12288k/}" "$ipq60xx_mk_path"
            echo "已将 ${device} 的 KERNEL_SIZE 更新为 12288k (12MB)"
        else
            echo "警告：未在 $ipq60xx_mk_path 中找到 Device/${device}" >&2
        fi
    done
}
